...
from dataset.activitynet_framelevel import ActivityNetCaptions_Frame, ActivityNetCaptions_ValFrame
...
def train_lstm(args):
    ...
    # transforms (not for temporal because whole clip is returned)
    ...
    train_dset = ActivityNetCaptions_Frame(args.root_path, ann_path='train_fps.json', spatial_transform=sp, temporal_transform=tp)
    trainloader = DataLoader(train_dset, batch_size=args.batch_size, shuffle=True, num_workers=args.n_cpu)
    max_train_it = int(len(train_dset) / args.batch_size)
    val_dset = ActivityNetCaptions_ValFrame(args.root_path, ann_path=['val_1_fps.json', 'val_2_fps.json'], spatial_transform=sp, temporal_transform=tp)
    valloader = DataLoader(val_dset, batch_size=args.batch_size, shuffle=True, num_workers=args.n_cpu)
...
def train_epoch(trainloader, encoder, decoder, optimizer, criterion, device, text_proc, log_interval, max_it):
    ...
    for it, data in enumerate(trainloader):
        # currently supports only batchsize of one
        ids = data['id'][0]
