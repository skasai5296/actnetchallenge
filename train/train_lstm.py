...
def train_lstm(args):
    ...
    trainloader = DataLoader(train_dset, batch_size=args.batch_size, shuffle=True, num_workers=args.n_cpu, collate_fn=collate_fn, drop_last=True, pin_memory=True)
    val_dset = ActivityNetCaptions_Val(args.root_path, args.framepath, n_samples_for_each_video=1, sample_duration=args.clip_len, spatial_transform=sp, temporal_transform=tp)
    valloader = DataLoader(val_dset, batch_size=args.batch_size, shuffle=True, num_workers=args.n_cpu, collate_fn=collate_fn, drop_last=True, pin_memory=True)
...
def train_epoch(trainloader, encoder, decoder, optimizer, criterion, device, text_proc, log_interval):
    ...
    for it, data in enumerate(trainloader):
        ...
        targets = captions.clone().detach().requires_grad(True)

        # flow through model
        feature = video_encoder(clip)
        # feature : (bs x C')

        output = caption_gen(feature, captions)
...
def validate(valloader, encoder, decoder, criterion, device):
    ...
    with torch.no_grad():
        for it, data in enumerate(valloader):
            ...
            output = caption_gen.sample(feature, captions)
